...
<hr>
<div style="text-align: right">Đông Hà, ngày {{$quote->created_at->day}} tháng {{$quote->created_at->month}} năm {{$quote->created_at->year}}</div>
<div style="text-align: center"><h1>BÁO GIÁ KHÁCH HÀNG</h1></div>
...
<div>Chúng tôi xin gửi đến Quý khách hàng bảng báo giá mặt hàng phụ tùng Howo như sau ( đơn giá chưa bao gồm thuế giá trị gia tăng VAT ): </div>
<div>
    <u>GHI CHÚ:</u>
...
<table border="1" width="100%" class="table-css">
    ...
    <tr style="text-align: center">
        <th style="width: 20px"><b>STT</b></th>
        <th style="text-align: center"><b>Tên SP</b></th>
        <th style="text-align: center">Hình ảnh</th>
        <th style="width: 20px; text-align: center"><b>ĐVT</b></th>
        <th style="width: 20px; text-align: center"><b>SL</b></th>
        <th style="text-align: center"><b>Đơn giá</b></th>
        <th style="text-align: center"><b>Thành tiền</b></th>
        @if($customVAT != "false")
            <th style="text-align: center"><b>Thuế suất</b></th>
        @endif
    </tr>
    </thead>
    <tbody>
            ...
            @if(count($products) < 150)
                <td style="width: 80px;">
                    @if ($product->image != null)
                        <img src="{{base_path('img/products/' . $product->image)}}" style="width: 100%;"/>
                    @endif
                </td>
            @endif
            <td>{{$product->type}}</td>
    ...
    <tr>
        ...
        <td style="text-align: right"><b>{{ number_format($totalItemPrice, 0) }}</b></td>
        @if($customVAT != "false")
            <td></td>
        @endif
    </tr>
    @if($totalDiscount < 0)
        <tr>
            <td colspan="6"><b>Giảm giá đơn hàng (5%)</b></td>
    ...
    @if($isVAT != false)
        <tr>
            <td colspan="6"><b>VAT</b></td>
            <td style="text-align: right"><b>{{ number_format($vat, 0) }}</b></td>
            @if($customVAT != "false")
                <td></td>
            @endif
        </tr>
    ...
    @if($showMaNSX != "false")
        <tr>
            <td colspan="6"><b>Nợ cũ</b></td>
            <td style="text-align: right"><b>{{ number_format($customer['money'], 0) }}</b></td>
            @if($customVAT != "false")
                <td></td>
            @endif
        </tr>
    ...
    <tr>
        ...
        <td style="text-align: right; background-color: yellow">
            <b>{{ number_format($totalItemPrice + ($showMaNSX != "false" ? $customer['money'] : 0) + ($isVAT != false ? $vat : 0) + $totalDiscount, 0) }}</b>
        </td>
        @if($customVAT != "false")
            <td></td>
        @endif
...
<div>2- Địa điểm giao hàng: Giao hàng tại kho Bên bán hoặc địa chỉ khách hàng yêu cầu trên địa bàn Hà Nội</div>
...
<table width="100%">
    <tr>
        <th style="text-align: center">Người lập phiếu</th>
        <th style="text-align: center">Xác nhận bên mua</th>
    </tr>
    <tr>
        <td style="text-align: center"><br><br><br><br><br><br><br><b>{{$user['full_name']}}</b></td>
        <td style="text-align: center"><br><br><br><br><br><br><br><b>{{$customer['name']}}</b></td>
    </tr>
</table>
</body>
</html>
